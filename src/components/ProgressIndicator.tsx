...
/**
 * 三步进度指示器组件
 * 展示 M1（分析需求）-> M3（构建故事线）-> M4（生成大纲）的执行进度
 */
        ...
        {STEPS.map((step, index) => (
          <div key={step.key} className="flex items-center">
            ...
            <div className="flex flex-col items-center text-center" style={{ minWidth: "90px" }}>
              ...
              <span className={`mt-2 text-sm font-medium ${getStepTextColor(progress[step.key])}`}>
                {step.label}
              </span>
            ...
            {index < STEPS.length - 1 && (
              <div
                className={`h-0.5 flex-1 mx-3 transition-colors duration-500 ${getConnectorColor(progress[step.key])}`}
                style={{ minWidth: "40px" }}
              />
            )}
          </div>
