...
### 六维质量检查清单（生成时逐项自查）
1. **需求契合**：每一页都必须服务于用户的核心需求"${intent.purpose}"，核心信息"${storyline.coreMessage}"必须贯穿全文
2. **逻辑连贯**：页面之间逻辑通顺，无跳跃或重复，章节过渡自然流畅，transitionToNext 要承上启下
3. **内容完整**：覆盖用户所有要点，不遗漏关键信息，每个章节的 keyMessage 必须在对应页面中体现
4. **信息密度**：每页信息量均衡，不出现过满（>5个内容块）或过空（<2个内容块）的页面
5. **受众匹配**：内容深度和表达方式适合"${intent.audience}"，术语和案例与受众认知水平匹配
6. **实用性**：所有内容具体可用，杜绝空洞套话，detail 字段至少20字且包含具体信息
